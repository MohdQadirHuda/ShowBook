...
const HomeScreen = () => {
  ...
  useLayoutEffect(() => {
    navigation.setOptions({
      headerLeft: () => (
        <Text style={{ fontSize: 17, fontWeight: "bold" }}>SHOWBOOK</Text>
      ),
      headerStyle: {
          ...
          >
            {languages.map((item, index) => (
              <Pressable
                ...
                onPress={() =>
                  setSelectedFilter(
                    item.language === selectedFilter ? null : item.language
                  )
                }
                style={{
                  margin: 10,
                  borderColor:
                    item.language === selectedFilter ? "orange" : "#CBCBCB",
                  borderWidth: item.language === selectedFilter ? 0 : 1,
                  backgroundColor:
                    item.language === selectedFilter ? "orange" : "transparent",
                  paddingVertical: 5,
                  borderRadius: 25,
              ...
              >
                <Text
                  style={{
                    color: item.language === selectedFilter ? "white" : "black",
                    fontWeight: "500",
                  }}
                >
                  {item.language}
